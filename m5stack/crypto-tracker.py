...
def getAPI():
  ...
  try:
    ...
    LTCPrice = round(json.loads((req.text))["data"]["LTC"]["quote"]["USD"]["price"],2)
    LTCMarketCap = round(json.loads((req.text))["data"]["LTC"]["quote"]["USD"]["market_cap"]/1000000000,2)
    ...
    result['ltcprice'] = LTCPrice
    result['ltcmarketcap'] = LTCMarketCap
    
    #lcd.print(str(result),5,10)
    #lcd.print(str(BTCPrice), 15, 10)
    ...
    result['ltcprice'] = 0
    result['ltcmarketcap'] = 0
    return result
   
while True:
  result = getAPI()
  #lcd.print(str(result['btcmarketcap']),5,10)
  redBackDrop()
